...
```{r, fig.align='center'}
# Heat Model Boxplot
boxplot(heat_trans_lm_small$residuals)
```

Cool Model Boxplot:
```{r, fig.align='center'}
# Cool Model Boxplot
boxplot(cool_trans_lm_small$residuals)
```

Heat Model Histogram:
```{r, fig.align='center'}
# Heat Model Histogram
hist(heat_trans_lm_small$residuals)
```

Cool Model Histogram:
```{r, fig.align='center'}
# Cool Model Histogram
hist(cool_trans_lm_small$residuals)
```

Heat Model Diagnostic Plots:
```{r, fig.align='center'}
autoplot(heat_trans_lm_small)
```

Cool Model Diagnostic Plots:
```{r, fig.align='center'}
autoplot(cool_trans_lm_small)
```

As the plots show, there is relatively normal distribution on the residuals, and this came after attempting many models to get the best fit for normality. Going in order, the break to the normality is most visible in the boxplots for the two models. We could not get rid of the outliers despite our best efforts. Moving on to the histograms, we see the normality curve take shape, especially with the cool model. Finally in the diagnostic plots, this was the best we could do to get the data as normally distributed as possible. This includes the least deviation on the Q-Q plot and randomized scatting of the residuals across the fitted values.
...
Below are the summaries for both the heat and cool models as well as interpretations for them.
